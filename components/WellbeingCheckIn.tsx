...
export const WellbeingCheckIn: React.FC<WellbeingCheckInProps> = ({ reason, onDismiss, onDismissForDay }) => {
  const messages: Record<NonNullable<WellbeingState['reason']>, { icon: React.ElementType; color: string; title: string; subtitle: string; message: string }> = {
    late_night: {
      ...
      subtitle: "Rest is important for clarity",
      message: "Analytical accuracy and intuition often decrease after 2 AM. Consider resting and reviewing your messages tomorrow for the best results."
    },
    same_person: {
      icon: Target,
      color: 'text-red-400',
      title: "Balance Reminder",
      subtitle: "Maintain your social bandwidth",
    ...
    high_frequency: {
      ...
      message: "You've been analyzing many interactions recently. A breather can help prevent fatigue and ensure your responses stay authentic."
    },
    high_risk: {
      icon: ShieldAlert,
      color: 'text-red-400',
      ...
      message: "Recent interaction patterns suggest this might be a stressful time. It might be good to take a moment for yourself."
    }
  };
  ...
  return (
    <div className="fixed inset-0 bg-black/90 backdrop-blur-xl z-[1000] flex items-center justify-center p-6 animate-fade-in font-sans select-none">
      <div className="bg-zinc-900 border border-white/5 max-w-lg w-full relative rounded-[3rem] shadow-2xl overflow-hidden">
        <div className="p-8 md:p-12">
          ...
          <div className="flex flex-col items-center text-center space-y-6 mb-10">
            ...
            <div>
              <h3 className="text-3xl font-bold text-white mb-2 leading-tight">{content.title}</h3>
              <p className={`text-[10px] font-bold ${content.color} uppercase tracking-widest`}>{content.subtitle}</p>
            </div>
          </div>

          <div className="space-y-10">
            <p className="text-zinc-400 text-base leading-relaxed text-center font-medium">
              {content.message}
            </p>
