...
export const getFeedbackEntries = (userId: string): FeedbackEntry[] => {
  try {
    const stored = localStorage.getItem(getUserKey(FEEDBACK_KEY, userId));
...
export const getSessionLogs = (userId: string): SessionLog[] => {
  try {
    const stored = localStorage.getItem(getUserKey(SESSIONS_KEY, userId));
    return stored ? JSON.parse(stored) : [];
  } catch (error) {
...
export const getWellbeingState = (userId: string): WellbeingState => {
  try {
    const stored = localStorage.getItem(getUserKey(WELLBEING_KEY, userId));
...
export const saveWellbeingState = (userId: string, state: WellbeingState): void => {
  try {
    ...
    logger.error('Failed to save wellbeing state:', error);
  }
};
